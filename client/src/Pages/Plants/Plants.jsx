...
export default function Plants() {
  ...
  }, [stableProducts, maxPrice, sortOrder, category]);

  console.log("From params:", category);
  console.log(
    "Filtered based on:",
    stableProducts.map((p) => p.category)
  );

  console.log(filteredProducts);

  // كل الكاتيجوريز اللي موجودة في الداتا (indoor / outdoor)
  const categoriesInData = useMemo(() => {
    return [...new Set(stableProducts.map((p) => p.category?.toLowerCase()))];
  }, [stableProducts]);

  // ❌ لو الـ category اللي في الـ URL مش موجود → روح لصفحة 404
  useEffect(() => {
    if (!category) return;

    const normalized = category.toLowerCase();

    if (!categoriesInData.includes(normalized)) {
      navigate("/404", { replace: true });
    }
  }, [category, categoriesInData, navigate]);
          ...
          <div className="flex flex-col items-center gap-4 lg:flex-row lg:justify-between lg:items-start">
            <div className="max-w-2xl">
              <p className="inline-flex items-center gap-2 mb-3 text-xs font-semibold tracking-widest uppercase text-secondary/80">
                ...
                {category === "indoor" ? "Indoor" : "Outdoor"} Collection
              </p>
          ...
          <div className="flex flex-col justify-between gap-3 px-4 py-3 rounded-xl bg-primary/12">
            <div>
              ...
              <p className="text-sm font-semibold text-secondary">
                {category === "indoor" ? "Indoor" : "Outdoor"} · Up to $
                {maxPrice}
              </p>
            </div>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="py-16 text-center rounded-2xl bg-secondary/2">
            <p className="text-lg font-medium text-secondary/70">
              No {category} plants found within this budget.
            </p>
          </div>
        )}
      </div>
